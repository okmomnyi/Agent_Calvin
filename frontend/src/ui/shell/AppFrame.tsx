...
import { StageCanvas } from "@/ui/stage/StageCanvas";
    ...
    <div className="flex h-full flex-col bg-bg text-text">
      ...
      <main className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-hidden p-3 md:grid-cols-[1fr_300px]">
        <div className="flex min-h-0 min-w-0 flex-col gap-3">
          {/* Glass card: the self-driving stage (Phase 38) lives here -- the ring, and
             whatever it's currently choreographing around, together with the waveform/mic
             control, so "is it listening" is never split across two unrelated panels.
             bg-surface-hi/60 + backdrop-blur is the glass effect -- the window itself stays
             opaque (its own body background), this reads as frosted glass against the
             app's own dark UI, not the desktop. StageCanvas degrades to exactly the old
             idle ring+waveform markup when there's no directive to show. */}
          <Card className="shrink-0 overflow-hidden border-line/60 bg-surface-hi/60 backdrop-blur-xl">
            <StageCanvas hudState={hudState} micLevel={micLevel} micOn={micOn} shell={shell} />
          </Card>
          {chat}
        </div>
